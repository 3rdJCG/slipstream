//! Vector BLF ingest — wraps `blf_asc::BlfReader` into [`FrameColumns`].
...
pub fn parse(path: &Path) -> Result<FrameColumns> {
    let mut reader = BlfReader::open(path).map_err(|e| Error::Parse(e.to_string()))?;
    ...
    // The iterator swallows parse errors and stops; surface them.
