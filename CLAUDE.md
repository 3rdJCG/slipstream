...
- **方針（2026-06-17、ユーザー確認済み）**: v0.3.1 のメモ化で ~50MB BLF が実用域になった。
  残るインジェスト系最適化（上記 P0 の BLF 並列展開 / Parquet キャッシュ / mmap、および
  行ストレージの `[u8;64]`→フラット `Vec<u8>`+offset 軽量化）は**最優先ではなく、今後の
  機能追加と並行して少しずつ**入れる。ロード処理そのものがボトルネックになった時に着手。
