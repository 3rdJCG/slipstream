...
- [x] 検索 / フィルタ（id / 時間範囲 / channel による。core: `FrameFilter` + `filtered_rows`/`filtered_count`。signal 値での絞り込みは今後）
- [ ] 統計（件数、min/max、平均、周期/周波数）
- [ ] ログ比較 / diff（2つのログを並べて表示。回帰/異常ビュー）
- [ ] CLI のみではなくファイルを開くダイアログ（`rfd`）
- [ ] 複数シグナルのプロット軸 / カーソル / 範囲ズーム

### P1 — フレーム健全性（周期/存在チェック）
フレームが想定どおりに来ているかを検査する。あくまで**フレーム単位（存在・周期）**
の検査であり、フレームの中身（ペイロード）を解釈するものではない。結果は
**Analysis タブ**で確認できる想定。DBC で周期が規定されているフレームはその周期を
基準に、規定がない（または DBC 自体が無い）場合は手動で条件を追加できる。
- [ ] DBC の周期属性（`GenMsgCycleTime` 等）を読み、フレームごとの期待周期を取得
- [ ] 実測周期の検査（欠落 / 遅延 / 過剰送信 を許容差付きで判定し、違反区間を列挙）
- [ ] DBC 未定義フレーム向けの手動ルール追加（対象 ID/名・期待周期・許容差を UI で設定）
- [ ] **ゲート条件**：健全性チェックを有効化する前提条件を設定可能にする
      （例：電源を表すシグナル値が ON の区間だけ周期チェックする等。シグナル値や
      時間範囲・channel を条件にできる）
- [ ] ゲート条件は**複数条件を AND/OR で組み合わせ**可能にする
      （例：電源 ON **かつ** 車速 > 0 の区間だけ検査）
- [ ] チェック結果を Analysis タブに表示（違反フレーム一覧 + 違反区間。行テーブルと連動）
- [ ] 健全性ルールセットの保存 / 読み込み（手動ルール + ゲート条件を 1 セットとして永続化）

### P1 — 画面構成（タブ UI シェル）
タブで画面を切り替える構成にする。各タブは同一の `core::Session` を共有し、タブ
自体は egui 側の薄いビュー（状態は `core` に置く 3 ルールを守る）。タブ名は仮称、
適宜変更してよい。
- [ ] タブ切り替えのシェル（トップレベルのタブバー + 各タブのビュー）
- [ ] **Config** タブ — BLF/ASC ファイルの読み込み、DBC の設定、インジェスト進捗の表示
- [ ] **Analysis** タブ — フレーム単位の解析（行テーブル / 検索・フィルタ / 統計）
- [ ] **Graph** タブ — デコード済みシグナルのプロット（軸 / カーソル / 範囲ズーム）
